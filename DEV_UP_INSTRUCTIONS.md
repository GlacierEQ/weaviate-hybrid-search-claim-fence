...
**Repository:** `GlacierEQ/weaviate-hybrid-search-claim-fence`  
**Company lens (independent):** Weaviate (`weaviate`)  
**Innovation:** Hybrid Search Claim Fence  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Weaviate affiliation, proprietary access, or production deployment.
...
Score support strength and fence claims below threshold as non-factual suggestions.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Weaviate employment, endorsement, or proprietary systems access.
...
File: `src/hybrid_search_claim_fence.py`

- Expand `HybridSearchClaimFence` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_hybrid_search_claim_fence.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_hybrid_search_claim_fence.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Weaviate APIs
