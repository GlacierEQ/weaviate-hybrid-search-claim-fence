"""Hybrid Search Claim Fence — SCAFFOLD STUB.

Company lens: Weaviate (independent; no affiliation).
Bottleneck: Hybrid BM25/vector answers over-claim support from weak hits.
...
class HybridSearchClaimFenceRequest:
...
@dataclass(frozen=True)
class HybridSearchClaimFenceReceipt:
    decision: Decision
    reasons: tuple[str, ...]
    digest: str
...
class HybridSearchClaimFence:
    """Central mechanism stub.

    Contract the filling AI must preserve:
    - `evaluate(req)` returns a HybridSearchClaimFenceReceipt
    ...
    def evaluate(self, req: HybridSearchClaimFenceRequest) -> HybridSearchClaimFenceReceipt:
        reasons: list[str] = []
        ...
        if reasons:
            ...
            return HybridSearchClaimFenceReceipt(
...
Mechanism = HybridSearchClaimFence
